...
export function containsAny<T>(collection: readonly T[], ...items: readonly T[]): boolean {
...
export function short_uuid(): string {
  const ta = Array.from(window.crypto.getRandomValues(buf.subarray(0, 16)).map((x) => (x % 26) + 65)).map((x) =>
