...
export const cssPath = function (node: Node, optimized?: boolean): string {
  if (node.nodeType !== Node.ELEMENT_NODE) {
    return "";
  }
...
// export const canGetJSPath = function (node: Node): boolean {
//   let wp: (Node | null) = node;
//   while (wp) {
//     const shadowRoot = wp.ancestorShadowRoot();
//     if (shadowRoot && shadowRoot.shadowRootType() !== Node.ShadowRootTypes.Open) {
//       return false;
//     }
...
// export const jsPath = function (node: Node, optimized?: boolean): string {
//   if (node.nodeType !== Node.ELEMENT_NODE) {
//     return "";
//   }
//   assert(node instanceof Element, "node should be an Element");
//   const path:string[] = [];
//   let wp: Node | null  = node;
//   while (wp) {
...
const cssPathStep = function (node: Node, optimized: boolean, isTargetNode: boolean): Step | null {
  if (node.nodeType !== Node.ELEMENT_NODE) {
    return null;
  }
  assert(node instanceof Element, "node should be an Element");
  ...
  for (let i = 0; siblings.length && (ownIndex === -1 || !needsNthChild) && i < siblings.length; ++i) {
  ...
  if (needsNthChild) {
    result += ":nth-child(" + (ownIndex + 1).toString(10) + ")";
